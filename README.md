...
Homebrew tap for Connextable projects.

## Install

```sh
brew install connextable/tap/<formula>
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/connextable/homebrew-tap/main/scripts/setup-deploy-key.sh | bash
```

The script detects the current GitHub repository, creates a `formula/<repo>`
write deploy key on `connextable/homebrew-tap`, and stores the private key as
...
```sh
curl -fsSL https://raw.githubusercontent.com/connextable/homebrew-tap/main/scripts/setup-deploy-key.sh | KEY_TITLE=formula/<formula> bash
```
...
| Setting | Value |
| --- | --- |
| Repository access | `connextable/homebrew-tap` |
...
```yaml
- uses: connextable/homebrew-tap/actions/publish/formula@main
  with:
    tap-path: tap
    source-path: source
    formula: <formula>
